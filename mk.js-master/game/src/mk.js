...
;(function () {
  ...
  mk.config = {
    ...
    FIGHTERS     : 'fighters/',
    STEP_DURATION: 80,
    PLAYER_TOP   : 480, // Ajusta este valor para alinear los personajes con el suelo
    PLAYER_SCALE : 1.7, // Escala los personajes para que sean más grandes
    BLOCK_DAMAGE : 0.2
  };
  ...
  mk.controllers.Network.prototype._initialize = function () {
    ...
    this._transport.on('response', function (response) {
      ...
      if (response === self.Responses.GAME_EXISTS) {
        alert('El nombre de juego ya existe. Usa otro nombre o únete como invitado.');
      } else if (response === self.Responses.GAME_NOT_EXISTS) {
        alert('El juego no existe. Si eres el host, responde "yes" cuando se te pregunte. Si eres invitado, asegúrate de que el host ya creó el juego y usa el mismo nombre.');
      } else if (response === self.Responses.GAME_FULL) {
        alert('El juego ya está lleno. Usa otro nombre de juego.');
      } else if (response !== self.Responses.SUCCESS) {
  ...
  mk.arenas.Arena.prototype.refresh = function () {
    ...
    for (var i = 0; i < this.fighters.length; i += 1) {
      f = this.fighters[i];
      var img = f.getState();
      var scale = mk.config.PLAYER_SCALE || 1;
      var w = img.width * scale;
      var h = img.height * scale;
      // Ajustar Y para que el "suelo" sea el mismo
      var y = f.getY() - (h - img.height);
      this._context.drawImage(img, f.getX(), y, w, h);
    }
  };
